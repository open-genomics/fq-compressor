...
|---|---|---|---|---|---|---|---|---|
| FQC v2 | 无参·流式 | 无（README.md#已知限制） | 2-bit 打包+异常列表 | 原样字节 | zstd 内部（LZ77+Huffman+FSE） | 无 | 2.96×（Illumina-like）/ 2.84×（ONT-like） | 压缩 53–56 / 解压 182–215 MiB/s |
...
- 与 SPRING 的分界线 = **read 重排序**（重排派吃跨 read 冗余，FQC 明确不做，README.md#已知限制）。
...
路线图阶段 I 把 §7"可借鉴①"落地为 `compress --quality-level N`（只影响质量流，codec ID 不变、解码零改动），并按 §7"可借鉴②"的准入门槛制度做判定：**吞吐回退 ≤10% 且压缩比改善 ≥3% 才合入默认值**（中位数，禁单次；吞吐为 5 次交错计时）。

体积扫描（64 MiB 输入，整个归档字节数；体积为确定性量，单次即准）：

| 质量 level | 11 符号随机质量 | 拟真质量（Q38–41 集中） |
|---|---:|---:|
| 1（默认） | 23,210,286 | 17,993,425 |
| 3 | 24,755,459（+6.7%） | 18,040,795（+0.3%） |
| 5 | 23,971,188（+3.3%） | 18,096,253（+0.6%） |
| 7 | 24,074,023（+3.7%） | 17,390,394（−3.3%） |
| 9 | 24,151,541（+4.0%） | 17,271,162（−4.0%） |
| 19 | 22,955,666（−1.1%） | 16,092,047（−10.6%） |

压缩吞吐回退（拟真质量，交错 5 次取中位数）：
