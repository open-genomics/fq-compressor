...
/// Wall-clock breakdown per pipeline stage. Each thread accumulates
/// `steady_clock` samples in locals and merges with one relaxed fetch_add at
/// exit, so the measurement stays off the synchronization hot path. Encoder
/// fields are the sum across all N workers. zstd lives in `encoderCompressNs`;
/// the writer should approach pure I/O (`writerWriteNs`).
struct StageTimings {
    ...
    std::uint64_t encoderCompressNs = 0;  // encoders: compressFrame (zstd x3)
...
/// frames, N encoder workers encode and zstd-compress frames in parallel, and
/// a writer thread emits them in frame-id order (via a reorder buffer). Two
/// bounded MPMC queues decouple the stages. Frame boundaries are naturally
/// independent (see ARCHITECTURE.md), so encoders need no shared mutable
/// state -- each encodes its own frame, tagged with a monotonic id the reorder
/// buffer uses to restore in-order submission to the writer.
